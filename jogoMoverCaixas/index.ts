...
let movingId: undefined | number = undefined;
let walls: NodeListOf<HTMLDivElement>;
let collidible: HTMLDivElement[] = [];
let canPush: HTMLDivElement[] = [];
...
const createCollisions = () => {
	...
	let actual: Collision = { x: wallsCopy[0].offsetLeft, y: wallsCopy[0].offsetTop, width: wallsCopy[0].offsetWidth, height: wallsCopy[0].offsetHeight }
	...
	for (let i = 0, actualIndex = 0; i < wallsCopy.length; i++) {
		actual.x = wallsCopy[i].offsetLeft; actual.y = wallsCopy[i].offsetTop;
		actual.width = wallsCopy[i].offsetWidth; actual.height = wallsCopy[i].offsetHeight;		
...
const checkBoxCollision = (box : HTMLDivElement) => {
	// WALLS COLLISION
	for (const col of collisions) {
...
const setEvents = () => {
	...
	document.addEventListener("keydown", (e) => {
		if (movingId === undefined) {
			if (["KeyD", "KeyA", "KeyS", "KeyW"].includes(e.code)) movingId = setInterval(move, gameSpeed);
		}
