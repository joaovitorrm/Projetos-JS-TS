...
socket.on("update_screen", (players : {name: string, icone: string, id: string, piece: string, wins: number, won : boolean, turn : boolean}[], activePlayers : number) => {    
    for (let i = 0; i < 2; i++) {
        if (!players[i]) {
...
socket.on("winner", (players : {name: string, icone: string, id: string, piece: string, wins: number, won : boolean, turn : boolean}[]) => {
    for (let i = 0; i < 2; i++) {
        winsAmounts[i].innerHTML = players[i].wins.toString();
        playersContainers[i].classList.remove("turno");
        if (players[i].won) {
